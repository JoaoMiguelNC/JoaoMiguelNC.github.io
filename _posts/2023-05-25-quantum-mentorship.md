...
In [1] the authors propose doing the optimization over the space of unitaries matrices instead of the parameter space.
...
$$T_U\mathrm{SU}(n)=\{V\in\mathbb{C}^{n\times n}\!: V^\dagger U + X^\dagger U = 0 \ \wedge \ \mathrm{tr}(U^\dagger V)=0\}$$

Thus, the corresponding Lie algebra, _i.e._, the tangent space at the identity, is
